...
class Token{
	...
	function add(){
		$this->db->add('tokens', array( 'token_key' => $this->tokenKey, 'user_id' => $this->userId, 'ip' => $this->ip, 'type' => $this->type, 'register_date' => date('Y-m-d H:i:s') ));
	...
	function clear(){
		...
		$query = 'DELETE FROM tokens WHERE register_date <= %s AND type=%s';
		$query = $this->db->prepare( $query, $date, $this->type );
		$this->db->execute( $query );
	}

	function check(){
		$query = 'SELECT token_id FROM tokens WHERE token_key=%s AND type=%s';
	...
	function getByIp(){
		$query = 'SELECT user_id FROM tokens WHERE token_key=%s AND ip=%s AND type=%s';
	...
	function getUserId(){
		$query = 'SELECT user_id FROM tokens WHERE token_key=%s AND type=%s';
		$query = $this->db->prepare( $query, $this->tokenKey, $this->type );
		$query = $this->db->getResult( $query );
		return isset( $query[0]['user_id'] ) ? $query[0]['user_id'] : 0 ;
	}
	...
	function delete(){
		$query = 'DELETE FROM tokens WHERE token_key=%s';
